...
class QuestionSendActivity : AppCompatActivity(), View.OnClickListener, DatabaseReference.CompletionListener {
    ...
    override fun onClick(p0: View?) {
       if(p0 === imageView){
       ...
       } else if(p0 === sendButton){
           // キーボードが出てたら閉じる
           val im = getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager
           im.hideSoftInputFromWindow(p0.getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS)
           ...
           if(title.isEmpty()){
               Snackbar.make(p0, getString(R.string.input_title),Snackbar.LENGTH_LONG).show()
           }

           if (body.isEmpty()) {
               // 質問が入力されていない時はエラーを表示するだけ
               Snackbar.make(p0, getString(R.string.question_message), Snackbar.LENGTH_LONG).show()
               return
           }
